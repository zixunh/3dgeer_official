...
__global__ void projection_geer_3dgs_fused_kernel(
    const uint32_t B,
    const uint32_t C,
    const uint32_t N,
    ...
    // projection using 3dgeer transform
...
void launch_projection_geer_3dgs_fused_kernel(
    // inputs
    ...
    projection_geer_3dgs_fused_kernel<float>
        <<<grid,
        threads,
        shmem_size,
